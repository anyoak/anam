...
def check_membership(user_id, force_check=False):
    ...
    max_retries = 3
    retry_delay = 2  # seconds - increased for stability
    
    for attempt in range(max_retries):
        try:
            # Check main channel membership with better error handling
            try:
                main_member = bot.get_chat_member(MAIN_CHANNEL, user_id)
                main_status = main_member.status
            except Exception as main_error:
                logger.error(f"Main channel check error: {main_error}")
                result = (False, "public")
                break
                
            if main_status not in ['member', 'administrator', 'creator']:
                result = (False, "public")
                break
                
            # Check backup channel membership with better error handling
            try:
                backup_member = bot.get_chat_member(BACKUP_CHANNEL, user_id)
                backup_status = backup_member.status
            except Exception as backup_error:
                logger.error(f"Backup channel check error: {backup_error}")
                result = (False, "private")
                break
                
            if backup_status not in ['member', 'administrator', 'creator']:
                result = (False, "private")
                break
...
# Debug command for membership checking
@bot.message_handler(commands=['debug_membership'])
def debug_membership(message):
    """Debug command to check membership status"""
    ...
    try:
        ...
        try:
            main_member = bot.get_chat_member(MAIN_CHANNEL, user_id)
            main_status = f"✅ Main Channel: {main_member.status}"
        except Exception as e:
            main_status = f"❌ Main Channel Error: {str(e)}"
        
        # Test backup channel
        try:
            backup_member = bot.get_chat_member(BACKUP_CHANNEL, user_id)
            backup_status = f"✅ Backup Channel: {backup_member.status}"
        except Exception as e:
            backup_status = f"❌ Backup Channel Error: {str(e)}"
        
        # Test membership function
        is_member, channel_type = check_membership(user_id, force_check=True)
        membership_status = f"Membership Check: {is_member}, Type: {channel_type}"
        
        debug_info = f"🔧 Membership Debug Info:\n\n{main_status}\n{backup_status}\n{membership_status}"
...
def show_main_menu(chat_id, user_id):
    try:
        ...
        if not countries:
            bot.send_message(chat_id, "❌ No numbers available at the moment. Please check back later.")
            return
        
        for country in countries:
            country_name = country[0]
            available = check_country_availability(country_name)
            btn_text = f"{country_name} ({available})" if available > 0 else f"{country_name} (0)"
        ...
        refresh_btn = types.InlineKeyboardButton("🔄 Refresh", callback_data="refresh_menu")
        ...
        try:
            ...
            if is_member:
                try:
                    ...
                    # If deletion fails, just edit the message
                    bot.edit_message_text(
                        "✅ Membership verified! Loading menu...", 
                        chat_id, 
                        message_id
                    )
                ...
                if channel_type == "public":
                    error_msg = "❌ You haven't joined the main channel yet! Please join and try again."
                elif channel_type == "private":
                    error_msg = "❌ You haven't joined the backup channel yet! Please join and try again."
                else:
                    error_msg = "❌ There was an error checking your membership. Please try again or contact support."
                
                # Show more specific instructions
                markup = types.InlineKeyboardMarkup()
                main_btn = types.InlineKeyboardButton("📢 Main Channel", url=f"https://t.me/{MAIN_CHANNEL[1:]}")
                backup_btn = types.InlineKeyboardButton("🔗 Backup Channel", url=BACKUP_CHANNEL_LINK)
                check_btn = types.InlineKeyboardButton("🔄 Check Again", callback_data="check_membership")
                ...
                try:
                    # Edit the existing message with updated information
                    bot.edit_message_text(
                        f"{error_msg}\n\n"
                        f"Please make sure you've joined both channels:\n"
                        f"• Main Channel: {MAIN_CHANNEL}\n"
                        f"• Backup Channel: Use the button below\n\n"
                        "After joining, click 'Check Again' to verify.",
                        chat_id,
                        message_id,
                        reply_markup=markup
                    )
                ...
                try:
                    bot.answer_callback_query(call.id, "Please join both channels and check again.")
                except:
                    pass
                    
        except Exception as e:
            logger.error(f"Error in check_membership callback: {e}")
            try:
