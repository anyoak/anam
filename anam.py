...
def get_admin_menu():
    keyboard = [
        [KeyboardButton(text="Set Minimum Withdraw"), KeyboardButton(text="Add Country Code")],
        [KeyboardButton(text="Update Prices"), KeyboardButton(text="View All Balances")],
        [KeyboardButton(text="Manage Numbers"), KeyboardButton(text="Delete All Balances")],
        [KeyboardButton(text="Export Data"), KeyboardButton(text="View Statistics")],
        [KeyboardButton(text="Set 2-Step Password"), KeyboardButton(text="Update Capacity")],
        [KeyboardButton(text="Update Confirmation Time"), KeyboardButton(text="Delete Country Code")],
        [KeyboardButton(text="View Balance by User")]
    ]
    return ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True)
...
async def set_min_withdraw_command(message: types.Message):
    ...
    args = message.text.split()
    if len(args) < 2:
        await message.answer("Provide an amount")
        return
    try:
...
async def view_statistics(message: types.Message):
    async with AsyncSession(engine) as session:
        ...
        total_accounts = await session.scalar(select(func.count()).select_from(Account))
